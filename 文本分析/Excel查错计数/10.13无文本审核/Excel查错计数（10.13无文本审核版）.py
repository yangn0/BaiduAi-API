...
for i in range(len(data.sheets())):
    ...
    headers=['文本段','中文总字数','总标点数','英文错词数','总英文单词数','英文字母数','DNN','积极情感概率','消极情感概率']
    x=0
    for cell in headers:
    ...
    for cell in l:
        ...
        try:
            if(type(cell)!=str):
                continue
            while(1):
                cell_flag=len(cell)/index_flag
                if(cell_flag>1):
                    num_flag+=1
                    dnn_f=DNNml(cell[index_flag-128:index_flag])
                    index_flag+=128
                    dnn+=dnn_f
                    continue
                else:
                     dnn_f=DNNml(cell[index_flag-128:])
                     #print(cell[index_flag-128:])
                     #print(dnn_f)
                     dnn+=dnn_f
                     dnn=dnn/num_flag
                     break
        ...
        worksheet.write(x,3,e[2])           #英文错词数
        worksheet.write(x,4,e[0])           #总英文单词数
        worksheet.write(x,5,e[4])           #英文字母数
        worksheet.write(x,6,dnn)            #DNN
        worksheet.write(x,7,se[0])          #积极情感概率
        worksheet.write(x,8,se[1])          #消极情感概率
